...
function createMongoUri() {
  ...
  if (process.env.MONGODB_USER && process.env.MONGODB_PASSWORD && process.env.DATABASE_SERVICE_NAME && process.env.MONGODB_DATABASE) { return `mongodb://${process.env.MONGODB_USER}:${process.env.MONGODB_PASSWORD}@${process.env.DATABASE_SERVICE_NAME}/${process.env.MONGODB_DATABASE}`; }
  return 'mongodb://localhost/network-server';
}
